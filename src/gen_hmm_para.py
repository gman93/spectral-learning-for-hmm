...
class Hmm:
    ...
    def learn_hmm(self, seq, no_of_hidden_states):
        ...
        self.oom_ax = np.zeros((self.no_of_hidden_state, self.no_of_hidden_state, self.no_of_observables),
                               dtype=np.float128)
        ...
        for i in range(seq.shape[1] - 1):
            ...
            if (i + 1 < seq.shape[1] - 1):
                tri[int(seq[0, i + 2] - 1), int(seq[0, i]) - 1, int(seq[0, i + 1]) - 1] = 1 + tri[
                    int(seq[0, i + 2]) - 1, int(seq[0, i]) - 1, int(seq[0, i + 1]) - 1]
        uni = uni / seq.shape[1]
        bi = bi / seq.shape[1]
        tri = tri / seq.shape[1]
